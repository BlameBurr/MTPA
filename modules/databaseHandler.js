	...
	constructor() {
		...
		this.prefix = {
			'get': (guildID) => {
				...
				statement = 'SELECT prefix from guilds WHERE guildID = ?';
			...
			'set': (guildID, prefix = defaultConfig.prefix) => {
				...
				statement = 'UPDATE guilds set prefix = ? WHERE guildID = ?';
		...
		this.guild = {
			'add': (guild) => {
				...
				statement = 'INSERT INTO guilds (guildId, name, region, prefix, invites) VALUES (?, ?, ?, ?, ?)';
