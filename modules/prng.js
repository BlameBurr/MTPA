...
class PRNG {
    ...
    xorShift(states = []) {
        if (states.length < 4) { // Checks if there are not four states
            if(!Array.isArray(states)) states = [this.lcg(states)*(2**32)];
            else states = [this.lcg()*(2**32)];
            for (let i = 1; i < 4; i++) {
                let result = this.lcg(this.seed(states[i-1]))*(2**32);
                states.push(result);
            } // Use the LCG algorithm to produce four random seeds
            console.log(states)
        }
        let number = new XORGen(states[0], states[1], states[2], states[3]).random(); // Run XORGenerator using the given states
        return number;
    }
}
