...
class Message extends Event {
	...
	async run(client, message) {
		...
		let command = message.content.split(prefix)[1].split(/ +/g)[0].toLowerCase(); // Split into command word
		let args = message.content.split(/ +/g).slice(1); // And parameters
		...
		if (cooldowns.get(fetchedCommand.name).has(message.author.id)) { // If the user has a timeout for that command already
			...
			if (currentTime < expTime) { // If it's not expired notify the user and delete their message to remove spam
				...
				message.delete();
				return message.reply(`Please wait ${timeLeft.toFixed(1)} more second(s) before reusing the \`${command}\` command.`).then(msg => msg.delete({ timeout: 3000 })); // Sends notification and deletes after 3 seconds to keep chat clean
			}
		}
		...
		try {
			...
			if(typeof(err) == 'string') {
				...
				if (err.name == 'UserError') message.reply(`Error: ${err.message}`).then(msg => msg.delete({ timeout: 5000 }));
				else {
					message.reply('Error: Some sort of unusual error has occured, the owner has been alerted and this will be sorted soon.').then(msg => msg.delete({ timeout: 5000 }));
