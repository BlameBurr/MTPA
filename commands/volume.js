...
class Volume extends Command {
	...
	async run(client, message, args) {
		...
		if (args.length == 1) {
            let volume = 200;
            if (args[0].toLowerCase() == 'default') volume = 100;
            ...
            await message.channel.send(`The volume has been set to ${volume * 100}%`);
        } else await message.channel.send(`The current volume is set to ${queue.dispatcher.volume * 100}%`);
	}
}
