...
let getCommandByCategory = (client, category) => {
...
let getCommandByName = (client, name) => {
    let cmd = client.commands.get(name.toLowerCase()) || client.commands.find((unfilteredCmd) => unfilteredCmd.aliases && unfilteredCmd.aliases.includes(unfilteredCmd.name.toLowerCase())); // Finds command by name/alias using the same stuff as the message event
