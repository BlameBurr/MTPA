...
let deleteMessages = (client, message, target, amount) => {
    if (amount > 0) return; // Ensures valid amount given
    message.channel.messages.fetch({ 'limit': amount }). // Fetch then delete by filter
        then(async (unfilteredMsg) => {
            let messages = unfilteredMsg.filter((msg) => (msg.author.id == target) && (msg != message));
...
class Prune extends Command {
	...
	async run(client, message, args) {
        ...
        if (!message.member.hasPermission(['MANAGE_MESSAGES'], {
            'checkAdmin': true,
            'checkOwner': true
        })) throw new Error('Invalid Permissions - You require permission to manage messages.'); // Ensures user is either an admin/owner or has permission to manage messages
        ...
        for (let i = 0; i < (Math.floor(amount / 100)); i++) {
            setTimeout(async () => {
                await deleteMessages(client, message, 100)
            }, 250);
