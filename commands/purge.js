...
class Purge extends Command {
	...
	async run(client, message, args) {
        ...
        if (!message.member.hasPermission(['MANAGE_MESSAGES'], {
            'checkAdmin': true,
            'checkOwner': true
        })) throw new Error('Invalid Permissions - You require permission to manage messages.'); // Check user permissions
        ...
        for (let i = 0; i < (Math.floor(amount / 100)); i++) {
            ...
            }, 250); // Bypass ratelimit and limit of 200 messages per bulkdelete
        }
